<?php
	if (isset($_SESSION['propertyissueTransferList'])) {
		$propertyTransferList = $_SESSION['propertyissueTransferList'];
		...
		for ($i = 0; $i < count($employeeList); $i ++) {
			$accountsName = $db->get('account_table',["last_name","middle_name","first_name","department"],['emp_id'=>$propertyTransferList[$i]->{'employee'}]);
			...
			<div class='cell padding20 shadow'>
		        	...
		        	<br />
								<span><small>Issued to</small></span><br/>
								<span class='sub-header place-left'>" . $accountsName['first_name'] . " " . $accountsName['last_name']
		            . "</span>
		            <p class='place-left'><small>" . $accountsName['department']  . " Department</small></p><br/>
								<br/>
								<br/>
								<br/>
								<hr class='thin' />";
